...
当非安全 LAN HTTP origin 没有提供原生 `crypto.randomUUID()` 时，Web 外壳会在加载客户端插件之前基于 `crypto.getRandomValues()` 安装 RFC 4122 version 4 实现。安全 origin 保留浏览器实现。这使同一份客户端图能够在公告的 LAN URL 上使用，同时不削弱 UUID 熵。
...
`dsh web` 的本地启动仍可通过 `http://127.0.0.1:3080` 访问；其他机器上的浏览器必须使用 `dsh web --host 0.0.0.0` 显式启用。CLI 尚未开放自定义接口地址或 IPv6 模式，而以编程方式使用承载层的消费方仍保留这种灵活性。服务器测试将环回模式和全接口模式向 Node 监听边界的传递固定为约定。组装浏览器测试将非 localhost HTTP origin 映射到 scaffold，并验证提供方发现启动时已经安装 UUID 兼容实现。
