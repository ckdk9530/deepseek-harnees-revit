...
**Reading configuration is as privileged as writing it.** `settings.describe` and `credentials.describe` join the privileged configuration set. It stays loopback-only by default; a deployment that accepts the risk may set `allowRemoteConfiguration: true` to grant the complete settings, credentials, and draft model-discovery plane to its declared `trustedHosts`. This is a trusted-network deployment choice, not authentication, and host-native plus preset-authoring operations remain loopback-only. The model catalog (`llm.providers`, `llm.models`) needs no opt-in: it carries provider ids, display names, and model lists — no endpoints, no key state — and a LAN client's model picker needs it. Both the default refusal and explicit grant are asserted over a real HTTP server rather than a hand-assembled request, because the `Host` header a browser actually sends is what decides them.
...
A LAN client on a `trustedHosts` deployment cannot render the settings page unless that deployment also enables `allowRemoteConfiguration`; the default remains loopback. Opting in exposes redacted reads, writes, credential status, write-only credential inputs, and caller-directed model discovery to every trusted authority, while undeclared authorities and host-native methods still receive 403. A plugin that registers a settings namespace is web-configurable without also registering a configurable provider, as superseded above. `SettingsDescriptor` carries a required `revision`, and `settings/document-updated` is an event any provider-side listener may observe. Clients that ignore `expectedRevision` keep last-write-wins semantics unchanged. Deferred: the fail-closed wire describe (with the `headers` and envelope-sanitization work it carries), and a non-executable browser schema protocol.
